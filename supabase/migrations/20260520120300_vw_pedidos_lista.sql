-- Cria as views que a tela /pedidos consome:
--   - public.vw_pedidos_lista: UNION de public.vw_pedidos (PROTHEUS/SALESFORCE/
--     SANKHYA) com crm.orcamentos (fonte='CRM'), enriquecida com atributos do
--     cliente (UF, tier, saude, score, financeiro).
--   - public.vw_pedidos_vendedores: DISTINCT vendedor_nome — alimenta o filtro
--     de vendedor sem esbarrar no cap default de 1000 linhas do PostgREST.
--
-- Campos cliente_dias_sem_compra, cliente_ticket_medio e cliente_faturamento_12m
-- ficam NULL: ainda não há fonte populada no schema. O front trata (num()→0).
--
-- security_invoker = on para que o RLS de crm.orcamentos / crm.clientes seja
-- aplicado com o JWT do caller, e não com o owner da view.
...
GRANT SELECT ON public.vw_pedidos_lista TO authenticated;
...
GRANT SELECT ON public.vw_pedidos_vendedores TO authenticated;
