...
import type { ColumnSettings as ColumnSettingsState } from "./useColumnSettings";

function SortableRow<TId extends string>({
  id,
  label,
  checked,
  onToggle,
...
export type ColumnSettingsProps<TId extends string> = {
  settings: ColumnSettingsState<TId>;
  labels: Record<TId, string>;
  fixedTop: TId[];
  triggerLabel?: string;
};

export function ColumnSettings<TId extends string>({
  settings,
  labels,
  fixedTop,
  triggerLabel = "Colunas",
}: ColumnSettingsProps<TId>) {
  ...
  function handleDragEnd(e: DragEndEvent) {
    ...
    settings.reorder(active.id as TId, over.id as TId);
  }

  const totalAll = settings.totalManageable + fixedTop.length;
  const visibleAll = settings.visibleManageableCount + fixedTop.length;
  ...
  return (
    <Popover>
      ...
      <PopoverContent align="end" sideOffset={8} className="w-[280px] p-2">
        ...
        <TooltipProvider delayDuration={200}>
          {fixedTop.map((id) => (
            <Tooltip key={id}>
              ...
              <TooltipContent side="left">Coluna fixa</TooltipContent>
            </Tooltip>
          ))}
        </TooltipProvider>
